...
void renderFunc()
{
	...
	if (uVertexTexPP != -1) //Se pone después de la activación del shader y antes del prostproceso
	{
		glActiveTexture(GL_TEXTURE0 + 1);
	...
	//glBlendFunc(GL_CONSTANT_COLOR, GL_CONSTANT_ALPHA);//Alfa constante lol
	//glBlendColor(0.8f, 0.8f, 0.8f, 0.3f); 
...
void initShaderPP(const char *vname, const char *fname){
	...
	if (!linked)
	{
		...
		std::cout << "Error en el enlazado del post proceso: " << std::endl;
		std::cout << "Error: " << logString << std::endl;
		delete logString;
		glDeleteProgram(postProccesProgram);
...
void resizeFBO(unsigned int w, unsigned int h){
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, w, h, 0,
		GL_RGBA, GL_UNSIGNED_BYTE, 0); //reservar espacio para la textura
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
